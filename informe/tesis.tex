  ...
  \section{\textbf{Programación por Restricciones}}
   ...
   \label{subsec:cspComplexity}
	...
	\subsection{Definición del problema como CSP}
		...
		Dados dos vectores $\vec{u} = (u_1, \dots, u_k)$ y $\vec{v} = (v_1, \dots, v_k)$, se dice que $\vec{u}$ \textit{domina}
		a $\vec{v}$ si y sólo si $\bigl<\forall i : i \in {1, \dots, k} : u_i \leq v_i\bigr>$.
		
		Dado un conjunto de vectores $S$, se dice que $\vec{v}^{\star} \in S$ es un \textit{óptimo de Pareto} si y sólo si
