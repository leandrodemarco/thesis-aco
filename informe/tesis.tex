...
%Fin tabla de contenidos
  ...
  actividades. A continuación se detallan cada una de estas actividades.
	...
	En esta sección se detallará la implementación de la metaheurística ACO\textsubscript{$\mathbb{R}$} para resolver el problema del filtro especificado
	en la sección (\ref{sec:fundelect}). Primero veremos por qué escogimos trabajar con dominios continuos para las variables en vez de
	discretos y luego discutiremos detalles referentes a la aplicación sobre el problema específicamente.
	
	\subsection{¿Por qué usar dominios continuos?}	
	Dado un problema combinatorio como el del filtro con un espacio de búsqueda $S$ y una \textit{función objetivo} $f$, 
	se puede definir una relación de vecindad  $N \subseteq S \times S$ que depende de la forma en que el conjunto de combinaciones es explorado. 
	
	Dos combinaciones $s_i$ y $s_j$  se consideran vecinas si la estrategia de búsqueda utilizada es tal que $s_j$ puede ser construida a partir de $s_i$ en	un sólo paso del proceso de búsqueda. 
	
	Diremos que una combinación $s_i$ es un \textit{óptimo local} si $f(s_i) \geq f(s_j), \forall s_j \in N(s_i)$. Si un óptimo local resulta
	ser mejor que todos los otros óptimos locales, entonces es una solución. Cuando los óptimos locales están distribuidos de tal forma
	que existe una correlación relativamente alta entre el valor de la función objetivo de una combinación y su distancia a una solución,
	el \textit{paisaje} de búsqueda se dice tener una forma \textit{Macizo Central}. En este caso, se puede utilizar la función objetivo
	para guiar la búsqueda.
	
	\subsection{Aplicación sobre el problema}
	Veremos bla bla bla ACO\textsubscript{$\mathbb{R}$}
