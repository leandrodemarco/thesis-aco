...
%Fin tabla de contenidos
  ...
  actividades. A continuación se detallan cada una de estas actividades.
	...
	elementales aquí para hacer el pseudo-código más legible.
	...
	\begin{itemize}
		...
		columnas
		\item $rand(m, n)$: dados $m < n$ devuelve un número aleatorio en el intervalo $[m,n]$
		...
		N[l-i][j],              & \text{caso contrario}
		\end{cases}
		\end{equation*} 
	\end{itemize}
	
		\begin{breakablealgorithm}
		\caption{ACO-$\mathbb{R}$ filtro}
		\label{alg:exhaustSearch}
		\begin{algorithmic}[1]
			...
			\State $f[0\dots2] \gets rand(R_{min}, R_{max})$
			\State $f[3\dots4] \gets rand(C_{min}, C_{max})$
			...
			\Procedure{HallarSoluciones}{}
			\State $hormiga \gets zeros(NumVars+1)$
			\State $T \gets repmat(hormiga, TamArchivo, 1)$
			...
			\EndFor
			\item[]
			\State $NuevaPobl \gets repmat(hormiga, TamMuestra, 1)$
			...
			\State $g \gets$ \Call{WheelSelection}{p} \Comment{Escoger núcleo Gaussiano}
			\State $NuevaPobl[t][i] \gets \mu[g][t] + \sigma[g][i]*rand()$
			\EndFor
